...
/**
 * Implementacion de la clase compañia servicio.
 */
@Service
public class CompanyServiceImpl implements CompanyService {
  ...
  public Mono<Company> updateCompany(String id, Company company) {
    return companyRepository.findById(id)
        ...
        .map(c -> {
          company.setId(id);
          if (company.getSigners() != null) {
            company.setSigners(company.getSigners());
          }
          if (company.getNameCompany() != null) {
            company.setNameCompany(company.getNameCompany());
          }
          if (company.getRuc() != null) {
            company.setRuc(company.getRuc());
          }
          return company;
        })
